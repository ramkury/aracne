...
void BrowserSocket::Initialize(uint16_t port) {
  ...
  int bindres = bind(welcome_socket_fd, (struct sockaddr *)&server_address,
...
void BrowserSocket::AwaitConnection() {
  ...
  client_socket_fd =
      accept(welcome_socket_fd, (sockaddr *)&client_address, &length);
  ...
  std::cout << "Accepted connection from " << inet_ntoa(client_address.sin_addr)
            << " on port " << ntohs(client_address.sin_port) << std::endl;
}
